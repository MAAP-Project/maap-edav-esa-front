...
import { Button, Checkbox, Descriptions, Empty, message, Popover, Spin, Tabs, Typography } from 'antd';
...
import { useCenterOnMapFromModule, useDataPaging, useDataSorting, useEntityCollectionList, useFormData, useMapSelection, useSelector } from '@oidajs/ui-react-mobx';
...
export const AdamOpensearchDatasetDiscoveryProviderResults = (props: AdamOpensearchDatasetDiscoveryProviderResultsProps) => {
    ...
    const tabs = useSelector(() => (
        Object.entries(props.provider.datasetQueries).map(([datasetId, query]) => {
            return {
                ...
                children: <AdamOpensearchDatasetDiscoveryResults queryState={query} onAddToMap={(item) => {
    ...
    return (
        <div className='adam-dataset-discovery-provider adam-opensearch-dataset-discovery-provider-results'>
            ...
            <Tabs
                activeKey={activeTab}
                items={tabs} onChange={(activeKey) => props.provider.setActiveResults(activeKey)}
                size='small'
            ...
            <Descriptions size='small' title='Source' column={1}>
                <Descriptions.Item label='Provider'>
                    {props.dataset.license.dataProviderUrl && isAbsoluteUrl(props.dataset.license.dataProviderUrl)
                        ? <a target='_blank' href={props.dataset.license.dataProviderUrl}>{props.dataset.license.dataProviderName}</a>
                ...
                {props.dataset.license.documentationURL && isAbsoluteUrl(props.dataset.license.documentationURL) &&
                    <Descriptions.Item label='Documentation'>
                        <a target='_blank' href={props.dataset.license.documentationURL}>{props.dataset.license.documentationURL}</a>
                    </Descriptions.Item>
                }
            </Descriptions>
        </div>
...
export const AdamOpensearchDatasetDiscoveryProviderQuery = (props: AdamOpensearchDatasetDiscoveryProviderQueryProps) => {
    ...
    const datasets = useSelector(() => props.provider.datasets.map((dataset) => {
        return (
            <div className='adam-opensearch-dataset-discovery-provider-query-dataset' key={dataset.datasetId}>
                ...
                <Popover content={<AdamOpensearchDatasetInfo dataset={dataset} />} trigger='click' ><InfoCircleOutlined /></Popover>
            </div>
        );
    }), [props.provider]);
    ...
    return (
        <div className='adam-opensearch-dataset-discovery-provider-query'>
            {datasetLoadingState === LoadingState.Error
                ? <Empty image={<CloseCircleOutlined />} description='Error retrieving datasets' imageStyle={{ fontSize: '30px', height: '40px' }} />
                : <React.Fragment>
                    <div className='adam-opensearch-dataset-discovery-provider-query-datasets'>
                        ...
                        <Spin spinning={datasetLoadingState === LoadingState.Loading} >
                            ...
                            : <div className='adam-opensearch-dataset-discovery-provider-query-datasets-items'>
                                {datasets}
                            </div>
                            }
                        </Spin>
                    ...
                    <Button type='primary' disabled={!canSearch} onClick={() => props.provider.search()}>Search</Button>
                </React.Fragment>
            }
        </div>
    );
